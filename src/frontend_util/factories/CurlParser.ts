...
import { ApiBodyFormData, ApiHeader, ApiStep, EnumApiBodyType, httpActionFromJSON } from '../ipc/api';
...
export class CurlParser {
    ...
    public parse(): Result<ApiStep> {
        ...
        try {
            ...
            apiStep.action = httpActionFromJSON((argv.request || "GET").toUpperCase())
            ...
            apiStep.headers =  _.sortBy(apiStep.headers, (header) => {
                return headerPriorities[header.key.toLowerCase()] || Number.MAX_SAFE_INTEGER;
            })
            if (apiStep.body) {
